...
class Tool(object):
	...
	def geocode(locationstring):
		urlraw = "https://api.openrouteservice.org/geocoding?query=" + locationstring + "&lang=de&limit=20&api_key=YOUR API CODE HERE"
	...
	def getRoute(start,stop,routeParams, messages):
		...
		urlraw += "&profile=" + routeParams[0] + "&preference=" + routeParams[1] + "&units=m&language=en&geometry=true&geometry_format=geojson&geometry_simplify=false&instructions=false&instructions_format=text&maneuvers=false&elevation=false&optimized=false&api_key=YOUR API CODE HERE"
		url=quote(urlraw, safe="%/:=&?~#+!$,;'@()*[]")
	...
	def execute(self, parameters, messages):
		...
		arcpy.AddMessage("Distance: " + str(route[0]["summary"]["distance"]) + "; Duration: " +  str(route[0]["summary"]["duration"]))
		###coordinates to log###
		return
